...
{
    /// <summary>
    /// TrimmingLayer2.xaml の相互作用ロジック
    /// </summary>
    public partial class TrimmingLayer2 : UserControl
    {
        public TrimmingLayer2()
        {
            InitializeComponent();
        }
        ...
        {
            var point = e.GetPosition(TrimmingLayer_view2);
            ...
            var margin = 20;
            var letherAreaLength = Math.Min(trimming.AssistRight - trimming.ViewLeft, trimming.AssistBottom - trimming.ViewTop);
            if (margin * 2 > letherAreaLength)
            {
                margin = 0;
            }

            if (point.X >= trimming.ViewLeft && point.X <= trimming.AssistRight && point.Y < (trimming.ViewTop + margin))
            {
                _dragLine = DragLine.Top;
                this.Cursor = Cursors.SizeNS;
            }
            else if (point.X >= trimming.ViewLeft && point.X <= trimming.AssistRight &&
                (point.Y > (trimming.AssistBottom - margin) || point.Y > (this.DesiredSize.Height - margin)))
            {
                _dragLine = DragLine.Bottom;
                this.Cursor = Cursors.SizeNS;
            }
            else if (point.Y >= trimming.ViewTop && point.Y <= trimming.AssistBottom && point.X < (trimming.ViewLeft + margin))
            {
                _dragLine = DragLine.Left;
                this.Cursor = Cursors.SizeWE;
            }
            else if (point.Y >= trimming.ViewTop && point.Y <= trimming.AssistBottom &&
                (point.X > (trimming.AssistRight - margin) || point.X > (this.DesiredSize.Width - margin)))
            {
            ...
            else if (point.X < trimming.ViewLeft && point.Y > trimming.AssistBottom)
            {
                _dragLine = DragLine.BottomLeft;
                this.Cursor = Cursors.SizeNESW;
            }
            else if (point.X > trimming.AssistRight && point.Y < trimming.ViewTop)
            {
            ...
            {
                TrimmingLayer_view2.CaptureMouse();
            }
        }
        ...
        private void TrimmingLayer_view_PreviewMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            ...
            TrimmingLayer_view2.ReleaseMouseCapture();
        }
